...
template <typename T>
void BSTNode<T>::pop(BSTNode<T>*& node, T& data)
{
...
{
  if (!root) // first push
    root = new BSTNode<T>(data, nullptr, nullptr);
...
template <typename T>
void BSTNode<T>::push(const T& data, BSTNode<T>* node)
{
  if (data < node->object)
  {
    if (!node->left) // No left. Insert data
      node->left = new BSTNode<T>(data, nullptr, nullptr);
  ...
  else // go right
  {
    if (!node->right) // No right. Insert data
      node->right = new BSTNode<T>(data, nullptr, nullptr);
