...
if [[ `awk '/(int|bool).*lessThan/, /^}/' appt.cpp | grep -v "//" | grep "lessThan *([^,]*,[^,]*)" | wc -l ` -ne 2 ]] ; then
 echo 'Design error: Call to lessThan() with two parameters not found in lessThan() in appt.cpp.'  >> designResults.txt
fi
...
if [[ `grep -v "for" *.cpp | grep ".*;.*;" | wc -l ` -ne 0 ]] ; then
  echo 'Fatal design error: multiple semi-colons on one line when not a for statement.'  >> designResults.txt
  (( ++ designError ))
fi 
