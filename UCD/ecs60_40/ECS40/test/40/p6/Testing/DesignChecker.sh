...
if ! awk '/class *Calendar/,/^} *;/' calendar.h | grep -v "//" | grep "Vector *< *Day *> *days *;" >& /dev/null ; then
 echo 'Design error: Vector <Day> not found in calendar.h'  >> designResults.txt
fi
...
if ! grep -v "//" day.h | grep "Appointment *\* *operator *>> *( *istream *&.*, *Day *&.*) *;" >& /dev/null ; then
...
if ! grep -v "//" DayOfWeek.h | grep "operator *==.*const *;" >& /dev/null ; then
 echo 'Design error: operator==...const not found in DayOfWeek.h'  >> designResults.txt
fi

if ! grep -v "//" DayOfWeek.h | grep "operator *<<.*const *DayOfWeek.*) *;" >& /dev/null ; then
 echo 'Design error: operator<<(..., const DayOfWeek...) not found in DayOfWeek.h'  >> designResults.txt
fi
...
if ! grep -v "//" DayOfWeek.h | grep "bool *operator *== *( *char.*)" >& /dev/null ; then
 echo 'Design error: "operator== (char...) not found in DayOfWeek.h'  >> designResults.txt
fi
...
if ! awk '/^LinkedList *< *T *> *:: *LinkedList/,/^}/' linkedlist.cpp | grep "// *LinkedList" >& /dev/null; then
...
if ! awk '/class *LinkedList/,/^} *;/' linkedlist.h | grep -v "//" | grep "ostream *& *operator *<<.*( *ostream *&.*, *const *LinkedList *<.*>.*) *;" >& /dev/null ; then
...
if ! awk '/class *LinkedList/,/^} *;/' linkedlist.h | grep -v "//" | grep "operator *+= *(" >& /dev/null ; then
...
if ! awk '/class *Vector/,/^} *;/' vector.h | grep -v "//" | grep "getCount *( *) *const" >& /dev/null ; then
 echo 'Design error: getCount() const not found in Vector class vector.h'  >> designResults.txt
fi


if ! awk '/class *Vector/,/^} *;/' vector.h | grep -v "//" | grep "Vector *( *)" >& /dev/null ; then
 echo 'Design error: default constructor not found in Vector class vector.h'  >> designResults.txt
fi

if ! awk '/class *Vector/,/public/' vector.h | grep -v "//" | grep "T *\* *array" >& /dev/null ; then
 echo 'Design error: private T *array not found in Vector class vector.h'  >> designResults.txt
fi

if ! awk '/class *Vector/,/^} *;/' vector.h | grep -v "//" | grep "~Vector *( *)" >& /dev/null ; then
 echo 'Design error: default destructor not found in Vector class vector.h'  >> designResults.txt
fi

if ! awk '/class *Vector/,/^} *;/' vector.h | grep -v "//" | grep "getCount *( *)" >& /dev/null ; then
 echo 'Design error: getCount not found in Vector class vector.h'  >> designResults.txt
fi

if ! awk '/class *Vector/,/^} *;/' vector.h | grep -v "//" | grep "T *& *operator *\[ *\] *( *int.*) *const *;" >& /dev/null ; then
 echo 'Design error: const operator[] not found in Vector class vector.h'  >> designResults.txt
fi

if ! awk '/class *Vector/,/^} *;/' vector.h | grep -v "//" | grep "T *& *operator *\[ *\] *( *int.*) *;" >& /dev/null ; then
 echo 'Design error: non-const operator[] not found in Vector class vector.h'  >> designResults.txt
fi

if ! awk '/class *Vector/,/^} *;/' vector.h | grep -v "//" | grep "operator *+= *(.*T *&.*) *;" >& /dev/null ; then
 echo 'Design error: operator+= (...T&...); not found in Vector class vector.h'  >> designResults.txt
fi

if ! awk '/class *Vector/,/^} *;/' vector.h | grep "^} *; *// *class *Vector" >& /dev/null ; then
 echo 'Design/Style error: "}; // class Vector not found at end of Vector class vector.h'  >> designResults.txt
fi



if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "WeeklyAppointment *( *const *WeeklyAppointment" >& /dev/null ; then
 echo 'Design error: "WeeklyAppointment( const" copy constructor not found in weeklyappt.h'  >> designResults.txt
fi
...
if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "class *WeeklyAppointment *: *public *Appointment" >& /dev/null ; then
 echo 'Design error: WeeklyAppointment not publicly derived from Appointment in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "" >& /dev/null ; then
 echo 'Design error: not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "WeeklyAppointment *( *)" >& /dev/null ; then
 echo 'Design error: Weekly Appointment default constructor not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "WeeklyAppointment *(.*WeeklyAppointment" >& /dev/null ; then
 echo 'Design error: Weekly copy constructor not found in weeklyappt.h'  >> designResults.txt
fi


if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "Appointment *\* *clone *( *) *const" >& /dev/null ; then
 echo 'Design error: "Appointment* clone() const" not found in weeklyappt.h'  >> designResults.txt
fi


if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "Appointment *\* *clone *( *)" >& /dev/null ; then
 echo 'Design error: "Appointment* clone()" not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "read *(" >& /dev/null ; then
 echo 'Design error: "read(" not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "write *(" >& /dev/null ; then
 echo 'Design error: "write( " not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "write *(.*) *const" >& /dev/null ; then
 echo 'Design error: "write(...) const " not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "getSeriesCount *( *) *const" >& /dev/null ; then
 echo 'Design error: "getSeriesCount() const" not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "char *\* *getSeries *( *) *const" >& /dev/null ; then
 echo 'Design error: "char* getSeries() const"not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "const *char *\* *getSeries" >& /dev/null ; then
 echo 'Design error: "const char* getSeries"not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "getSeriesCount *( *)" >& /dev/null ; then
 echo 'Design error: "getSeriesCount()" not found in weeklyappt.h'  >> designResults.txt
fi

if ! awk '/class *WeeklyAppointment/,/^} *;/' weeklyappt.h| grep -v "//" \
  | grep "char *\* *getSeries *( *)" >& /dev/null ; then
 echo 'Design error: "char* getSeries()"not found in weeklyappt.h'  >> designResults.txt
fi
...
if grep "Appointment" linkedlist.* >& /dev/null; then
 echo 'Fatal design error: "Appointment" found in linkedlist.*'  >> designResults.txt
 (( ++ designError ))
fi

if grep "Day" vector.* >& /dev/null; then
 echo 'Fatal design error: "Day" found in vector.*'  >> designResults.txt
 (( ++ designError ))
fi

if ls *.cpp | egrep -v "(calendar.cpp|main.cpp|day.cpp|appt.cpp|time.cpp|DayOfWeek.cpp|linkedlist.cpp|weeklyappt.cpp|vector.cpp)" >& /dev/null; then
 echo 'Fatal design error: *.cpp found other than specified by assignment found'  >> designResults.txt
 (( ++ designError ))
fi
