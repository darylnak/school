...
rm -f appts.csv DOW.dat in*.txt out*.txt >& /dev/null
cp $testerDirectory/appts.csv $testerDirectory/DOW.dat $testerDirectory/in*.txt . 
...
while [[ $fileNum -lt 7 ]]; do
  ...
  (calendar.out appts.csv < $testerDirectory/in$fileNum.txt | tail -40 > out$fileNum.txt )&
  sleep 2
...
echo "Memory leak check (3 points):" >> operationsResults.txt
rm -f out$fileNum.txt >& /dev/null
(valgrind calendar.out appts.csv < $testerDirectory/in6.txt >& out$fileNum.txt )&
sleep 2
...
if [[ -s out$fileNum.txt ]]; then
  ...
  if ! grep "ERROR SUMMARY: 0 errors from 0 contexts" out$fileNum.txt >& /dev/null ; then
    ...
    echo "OK +1" >> operationsResults.txt
    (( opScore += 1 ))
  fi
else
