...
if ! awk ' /class/, /^} *;/ ' stack.h | grep "push.*(.*const" >& /dev/null ; then
  echo '"push(const" not found in stack.h.'  >> designResults.txt
fi


if ! awk ' /class/, /^} *;/ ' queue.h | grep "push.*(.*const" >& /dev/null ; then
  echo '"push(const" not found in stack.h.'  >> designResults.txt
fi

if ! awk ' /class *BST *$/, /^} *;/ ' BST.h | grep "push *(.*const *T[&a-zA-Z ]*)" >& /dev/null ; then
  echo '"push(...const T)" not found in BST.h.'  >> designResults.txt
fi


if ! awk ' /class *BST *$/, /^} *;/ ' BST.h | egrep "push.*(const *T.*BSTNode|BSTNode.*const *T)" >& /dev/null ; then
  echo '"Either "push...const T...BSTNode", or "push...BSTNode...const T" not found in BST.h.'  >> designResults.txt
fi
