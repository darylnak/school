...
{
  if(t) // check for !NULL/past leaf
  {
    int leftHeight  = 1 + height(t->left); // get height of left child
    int rightHeight = 1 + height(t->right); // get height of right child

    if(leftHeight > rightHeight)  // return greater height to parent
      return leftHeight;
    else
      return rightHeight;
  }

  return -1;  // offset because leaves are at height 0
}
...
int BinarySearchTreeX<Comparable>::sum(BinaryNodeX<Comparable> *t) const
{
  ...
  return 0; // no offset because we want value of element
}
...
{
  if(x == possibleAncestor) // check if same number passed
    return true;

  return find(x, find(possibleAncestor, root));
  // find potential ancestor, then find x using potential ancestor as root
}
...
{
  // height of full level is simply the difference between height of tree and depth of level.
  return height() - deepestFull();
}
...
{
  if(!t->left || !t->right) // node is not full. Do not contribute to height count.
    return 0;

  if(deepestFull(t->left) > deepestFull(t->right)) // return min(depth(left), depth(right))
    return 1 + deepestFull(t->right); // get height of full node
  else
    return 1 + deepestFull(t->left); // get height of full node
}